def movie_organizer(*args):
    movies = {}
    for movie in args:  # get all movies and add to dictionary
        if movie[1] not in movies:  # create genre if it does not exist yet
            movies[movie[1]] = []  # create empty list for movies in genre
        movies[movie[1]].append(movie[0])  # add movie to genre
    for genre in movies:  # sort movies in genre alphabetically
        movies[genre].sort()
    movies = dict(sorted(movies.items(), key=lambda x: (-len(x[1]), x[0])))  # unpack the dict and
    result = ""  # sort by number of movies in genre and then by genre name alphabetically
    for genre in movies:  # sort by number of movies in genre and then by genre name alphabetically
